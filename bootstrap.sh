...
##########################################################################
#
# Create a GKE cluster with Workload Identity enabled and 2 node pools:
# - The default node pool will run the GKE system workloads
# - The spot-pool will use SpotVMs and run the Cronjobs
#
# Optionally set the cluster version by adding: --cluster-version "VERSION" \
...
if ! gcloud artifacts repositories list --location="${REGION}" --project="${PROJECT_ID}" --format="value(REPOSITORY)" 2>/dev/null | grep "${REPO_NAME}" >/dev/null; then
  ...
  gcloud artifacts repositories create "${REPO_NAME}" \
      ...
      --location="${REGION}" \
      --description="Default repository for Helm charts" \
      --immutable-tags \
      --project="${PROJECT_ID}"
...
if ! gcloud storage buckets list --project="${PROJECT_ID}" --format="value(NAME)" 2>/dev/null | grep "gs://${APP_NAME}-compliance-reports" >/dev/null; then
  ...
  gcloud storage buckets add-iam-policy-binding "gs://${APP_NAME}-compliance-reports" \
      --member "serviceAccount:${GCP_SA_NAME}@${PROJECT_ID}.iam.gserviceaccount.com" \
